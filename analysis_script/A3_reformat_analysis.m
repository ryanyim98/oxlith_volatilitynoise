...
to_learn = "time";  %"movement" "time"

load([data_path+to_learn+'_data_learned_all_-14toEnd_allparticipants.mat']);
...
%% export parameters to csv for further statistical analysis
participant_data = {};
all_data_out = {};
for participant = 1:size(alldata.subjectdata,2)
...
writetable(all_data_out,[data_path+to_learn+"_data_HLM_-14toEnd_allparticipants.csv"]);

%% organise into one matrix for further analysis in matlab
if to_learn ~= "mood"
...
exportgraphics(gcf,[data_path+data + ".png"],'Resolution',300)
...
exportgraphics(gcf,[data_path + data + "_spaghetti.png"],'Resolution',300)
%% t test prep
