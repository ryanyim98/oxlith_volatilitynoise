...
for participant=1:size(demo,1)
        ...
        alldata.subjectdata(participant).allData = movement_data(string(cell2mat(movement_data.ID)) == string(alldata.subjectdata(participant).ID),[5:13 24:25]);
...
% make data that aligns at day 0
for participant=1:size(demo,1)
...
for participant=1:size(demo,1)
    alldata.subjectdata(participant).allData = alldata.subjectdata(participant).allData((alldata.subjectdata(participant).allData.Day >= daysKept(1) & alldata.subjectdata(participant).allData.Day <= daysKept(2)),:);
    alldata.subjectdata(participant).allData.Time5 = (alldata.subjectdata(participant).allData.Time5 - min_Time5)/(max_Time5-min_Time5)*0.6+0.2;
...
for participant=1:size(demo,1)
    alldata.subjectdata(participant).allData = alldata.subjectdata(participant).allData((alldata.subjectdata(participant).allData.Day >= daysKept(1) & alldata.subjectdata(participant).allData.Day <= daysKept(2)),:);
    alldata.subjectdata(participant).allData.pos = (alldata.subjectdata(participant).allData.pos - min_pos)/(max_pos-min_pos)*0.6+0.2;
