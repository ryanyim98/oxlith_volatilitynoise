...
ptc2 <-ggplot(df_hlm_tc, aes(x = day, y = m_movement_M10_vmu, color = Allocation))+
  geom_line()+
...
ptc3 <-ggplot(df_hlm_tc, aes(x = day, y = m_time_M10_mean, color = Allocation))+
  geom_line()+
...
ptc4<- ggplot(df_hlm_tc, aes(x = day, y = m_time_M10_vmu, color = Allocation))+
  geom_line()+
...
summary(mod_movement_mean_M10)$coefficients
...
l1 <- lm(M10_change ~ M10_vmu_change + Age + Gender, df_hlm_change)
summary(l1)
lm.beta::lm.beta(l1)

summary(lm(timeM10_change ~ timeM10_vmu_change + Age + Gender, df_hlm_change))
lm.beta::lm.beta(lm(timeM10_change ~ timeM10_vmu_change + Age + Gender, df_hlm_change))

summary(lm(timeM10_vmu_change ~ M10_vmu_change + Age + Gender, df_hlm_change))
lm.beta::lm.beta(lm(timeM10_vmu_change ~ M10_vmu_change + Age + Gender, df_hlm_change))

summary(lm(timeM10_change ~ M10_change + Age + Gender, df_hlm_change))
lm.beta::lm.beta(lm(timeM10_change ~ M10_change + Age + Gender, df_hlm_change))
...
pcorr1 <- ggplot(df_hlm_change, aes(y = M10_change, x = M10_vmu_change))+
  ...
  scale_color_manual(values = orangeNavyPalette)+
  annotate("text",y = 20 ,x = -0.5, label = "r = -0.308, p = .199, N = 24")+
  labs(x = "change in M10 volatility",
       y = "change in M10 level")
...
pcorr2 <-  ggplot(df_hlm_change, aes(y = timeM10_vmu_change, x = M10_vmu_change))+
  ...
  geom_point(aes(color = Allocation))+
  scale_color_manual(values = orangeNavyPalette)+
  annotate("text",x = -0.5,y = 1, label = "r = 0.402, p = .034, N = 33")+
...
ggcorrplot::ggcorrplot(corr_lm,type = "upper",p.mat = corr_lm_pmat,
                       colors=c("orange", "white", "steelblue4"),pch = 4,lab_size = 5,
                        lab = TRUE, digits = 2,hc.method = "complete")+
  labs(title = "Correlations among amount of change (week4 - baseline)",
       subtitle = "Two treatment groups combined (N = 34)")+
ggcorrplot::ggcorrplot(corr_lm2,type = "upper",p.mat = corr_lm_pmat,
                       colors=c("orange", "white", "steelblue4"),pch = 4,lab_size = 5,
                        lab = TRUE, digits = 2,hc.method = "complete")+
  labs(title = "Correlations among amount of change (week4 - baseline)",
...
(p4a_M10 + p11b_M10) / (p4b_M10 + p12b_M10) / (pcorr1+pcorr2)+
  plot_annotation(tag_levels = "a")
ggsave("../figs/FIG4_movement_n_time.png",width = 8 ,height=10,dpi = 300)
